...
void main() {
  ...
  group('buildUri', () {
    test('uses the SDMTS server root + api/where', () {
      final uri = transportWith(unused)
          .buildUri('arrivals-and-departures-for-stop', id: 'MTS_24151');
      expect(
        uri.toString(),
    ...
    test('adds a missing trailing slash to baseUrl', () {
      final uri = transportWith(
        unused,
        base: 'https://realtime.sdmts.com/api',
  ...
  group('decoding order', () {
    Future<Object?> errorFor(http.Response response) async {
      try {
        await transportWith(MockClient((_) async => response))
            .get('stop', id: 'X');
        return null;
      } catch (e) {
        return e;
    ...
    test('3. non-JSON 200 body is ObaFormatException', () async {
      expect(
        await errorFor(http.Response('not json', 200)),
    ...
    test('4. envelope code != 200 is envelope error with text', () async {
      final e = await errorFor(
        http.Response(fixture('error_envelope.json'), 200),
      );
      expect(
        e,
        isA<ObaApiException>()
    ...
    test('any other Exception (e.g. TLS during body read) is '
      ...
      await expectLater(
        transportWith(client).get('stop', id: 'X'),
        throwsA(
          isA<ObaNetworkException>().having(
            (e) => e.cause,
            'cause',
            isA<_TlsLike>(),
          ),
    ...
    test('toList parses list responses and flags', () {
      final envelope = Transport.decode(
        http.Response(
